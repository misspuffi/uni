 ...
 * @returns {Promise<object>} - 手机号三要素核验结果
 * @constructor
 */
function PhoneVerification(params) {
  if (!params.phone||!params.idCard||!params.name) {
    throw new Error('姓名、身份证号和手机号必须都传');
  }

  return apis.phoneVerification(params);
}
function DetectAuth(params) {
  if (!params.ruleId) {
    throw new Error('RuleId必须传');
  }

  return apis.DetectAuth(params);
}
export { checkIdCardInformation, IdCardOCRVerification, PhoneVerification, DetectAuth };
