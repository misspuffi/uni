		...
		<view class="pd-md">
			<view class="pl-lg pr-lg pb-lg fill-base radius-16 mb-md" v-for="(item,index) in list.data" :key="index">
				<view class="item-nav flex-between ">
					...
					<text :style="{color:primaryColor}" v-if="item.status == 1">已拉黑</text>
					<text class="c-caption" v-else>已取消拉黑</text>
				</view>
				<view class="item-cont radius-16 pd-lg">
					<view class="f-paragraph text-bold pb-md">拉黑理由</view>
					<view class="c-paragraph ellipsis-2" style="white-space: pre-wrap;">{{item.content}}</view>
				</view>
        <button class="button black" @click="blackNoConfirm(item.id)" :style="{background:primaryColor,color:configInfo}">{{item.status?'取消拉黑':'拉黑'}}</button>
			</view>
		</view>
...
<script>
	...
	export default {
      ...
      blackNoConfirm(id) {
        ...
        console.log(params)
        this.$api.technician.updateBlack(params).then(res => {
          this.$util.hideAll()
          this.initIndex()
        })
      },
			...
			async getList() {
				...
				let newList = await this.$api.technician.blackList(param)
